    ...
    while(True):
        ...
        try:
            ...
            if contador==0:
                ...
                strQuery="Insert into alianza_inac (idAlianza,NombreAlianza,id_server,created_at,updated_at) VALUES ("+id_alianza+",'"+ nombre_alianza +","+str(id_server)+"',CURDATE(),CURDATE())"
                print(strQuery)
                cursor2.execute(strQuery)
            else:
                strQuery="UPDATE alianza_inac SET NombreAlianza='"+nombre_alianza+"',updated_at=CURRENT_DATE() WHERE idAlianza = "+id_alianza+"and id_server = "+str(id_server)
                cursor2.execute(strQuery)
            ...
            if contador==0:
                #Insertar nueva alianza 
                    strQuery="INSERT INTO cuenta_inac(IdCuenta,IdServer,IdAlianza,NombreCuenta,Raza,Activo,odserver,created_at,supend_at, modif_at) VALUES ("+str(id_cuenta)+","+str(id_server)+","+str(id_alianza)+",'"+ nombre_cuenta+"',"+str(id_raza)+","+str(id_server)+",'1',CURDATE(),NULL,CURDATE())"
                    #print(strQuery)
                    cursor2.execute(strQuery)  
            #ALDEAS  
                #Insertar nueva alianza 
            strQuery="INSERT INTO aldea_inac(IdAldea,NombreAldea,coord_x,coord_y,poblacion,id_server,created_at,updated_at,IdCuenta) VALUES ("+id_aldea+",'"+nombre_aldea+"',"+coord_x+","+coord_y+","+poblacion+","+str(id_server)+",CURDATE(),CURDATE(),"+str(id_cuenta)+")"
            #print(strQuery)
            cursor2.execute(strQuery)
        except: 
           #print(contador2)
